...
const OnboardingScreen = () => {
    ...
    const handleGallery = async () => {
        ...
        if (!result.canceled && result.assets[0]) {
            console.log('[OnboardingScreen] Media selected from gallery:', result.assets[0].uri);
    ...
    const handleCamera = async () => {
        ...
        if (!result.canceled && result.assets[0]) {
            console.log('[OnboardingScreen] Media captured from camera:', result.assets[0].uri);
            setPhotoUri(result.assets[0].uri);
        }
    };
    ...
    const handleFinish = async () => {
        ...
        if (!isAgent) {
            updates.specialization = selectedCategories.join(', ');
            updates.service_type = selectedCategories[0] || null;
        } else {
            updates.specialization = selectedSpecialization;
            updates.bio = bio;
            updates.years_exp = parseInt(yearsExp, 10);
        }

        if (photoUri) {
            const extMatch = photoUri.match(/\.([a-zA-Z0-9]+)(?:\?|$)/);
            ...
            try {
                ...
                const storagePath = await uploadToStorage('portfolios', filename, photoUri);
                const { data: publicUrlData } = getPublicStorageUrl('portfolios', storagePath);
                updates.gallery = [{ url: publicUrlData.publicUrl, type: isVideo ? 'video' : 'image' }];
            } catch (error) {
                console.warn('[OnboardingScreen] Failed to upload portfolio photo, saving local URI instead', error);
                updates.gallery = [{ url: photoUri, type: isVideo ? 'video' : 'image' }];
            }
        }
            ...
            <Animated.View style={[styles.content, { transform: [{ translateX: slideAnim }] }]}>
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                    ...
                    {isAgent && step === 1 && (
                        <View>
                            <Text style={styles.title}>Where are you based?</Text>
                            <Text style={styles.subtitle}>This helps customers find you nearby</Text>
                            <View style={styles.chipContainer}>
                                {LOCATIONS.map((loc) => (
                                    <TouchableOpacity
                                        key={loc}
                        ...
                        <View>
                            ...
                            <View style={styles.uploadContainer}>
                                {photoUri ? (
                                    ...
                                    <TouchableOpacity style={styles.dashedBox} onPress={handleGallery} activeOpacity={0.8}>
